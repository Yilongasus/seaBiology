...
<template>
    ...
    <div id="main" class="my-4 p-5 position-sticky zindex_2 text-white">
        ...
        <div class="container mt-1 pt-3">
            <div v-if="seaData.length" class="pb-4 d-grid">
                <div
                ...
                >
                    <a
                        :href="data['Source']"
                        class="fs-6 d-block my-2 z-1 position-relative overflow-hidden text-decoration-none"
                    >
                        <div
                        class="d-flex flex-row justify-content-between align-items-center px-5 py-3 text-center rounded"
                        style="
                        ...
                        >
                        <p
                            class="col-span-2 fs-4 fw-semibold pt-3"
                            style="color: #163464"
                        >
                            {{ data["標題"] }}
                        </p>
                        <div class="d-flex flex-column justify-content-center">
                            <p class="text-start col-span-2 fs-5" style="color: #756c63">
                            發佈日期
                            <span class="badge text-wrap">{{ data["上版日期"] }}</span>
                            </p>
                            <p class="text-start col-span-2 fs-5" style="color: #756c63">
                            資料來源
                            <span class="badge text-wrap">{{ data["資料來源"] }}</span>
                            </p>
                        </div>
                        </div>
                        <div
                        class="position-absolute h-100 w-100 overflow-hidden bg-black opacity-75 rounded-lg mask-block z-2"
                        ></div>
                        <div
                        class="position-absolute align-items-center rounded-lg justify-content-center mask-flex z-2"
                        >
                        <span class="px-5 py-2 rounded">
                            前往查看
                            <box-icon
                            class="bx bxs-like bx-fade-right"
                            animation="fade-up"
                            color="#ffffff"
                            type="solid"
...
export default {
    ...
    axios.get("/api/OpenData.aspx?SN=C7A4275096465BFB").then((res, error) => {
        if (error) {
